...
│
├─ 📊 配置层 (data/)            ── 纯数据，可任意改
│   ├─ GameConfig.ts            全局参数（速率/阈值/网格/传送带容量）
│   ├─ FoodData.ts              3 种食物数值
│   ├─ DialogueData.ts          4 类台词池
│   └─ BuildableDef.ts          8 种可建造建筑
│
├─ 🧠 纯逻辑层 (core/)           ── 不依赖 Phaser，可单测（100 个 vitest 测试）
│   ├─ dragon/
│   │   ├─ DragonState.ts       数据 + mood 计算属性
│   │   └─ DragonLogic.ts       饥饿/满意度 tick + feed
│   └─ factory/
│       ├─ Item.ts              7 种物品类型
│       ├─ Recipe.ts            3 个配方（唯一来源）
│       ├─ MachineStatus.ts     状态枚举
│       └─ FactoryWorld.ts      ⭐ 工厂引擎（Source/Machine/Belt/Feeder）
│
├─ ⚙️ 系统层 (systems/)          ── 工厂子系统 + 横切
│   ├─ ScaleSystem.ts           龙鳞自动产出
│   ├─ FactoryRenderer.ts       ⭐ 工厂渲染（静态+动态+模式覆盖）
│   ├─ BuildSystem.ts           ⭐ 建造/拆除/拖拽/网格
│   └─ BeltEditor.ts            ⭐ 传送带连接/删除/端口检测
│
├─ 🎨 渲染层 (entities/)         ── 龙宝宝视觉
│   └─ dragon/
│       ├─ Dragon.ts            🐉 龙渲染（162行，动画委托 DragonAnimations）
│       ├─ DragonAnimations.ts  ⭐ 7 种动画管理（250行）
│       ├─ IdleBehaviors.ts     空闲行为权重表
│       └─ SpeechBubble.ts      气泡（带 follow）
│
├─ 🎬 场景层 (scenes/)           ── Phaser Scene，做粘合
│   ├─ BootScene.ts             🚀 启动 + 共享数据
│   ├─ DragonScene.ts           🐉 龙宝宝主场景
│   ├─ FactoryScene.ts          🏭 工厂编排层（304行，从 1275 行精简）
│   └─ HUDScene.ts              📊 HUD + 驱动 GameClock
│
├─ 🧩 UI 组件 (ui/)              ── 可复用 UI
│   ├─ InventoryHUD.ts          物品栏显示
│   ├─ BuildMenu.ts             建造工具栏
│   └─ RecipePanel.ts           食谱选择面板
│
├─ 📡 events/EventBus.ts        全局事件总线
├─ ⏱ utils/GameClock.ts         10Hz 逻辑帧
└─ 🚀 main.ts                   Phaser Game 入口（含全局错误边界）
```
...
### ⚠️ 改进空间（2026-06-19 已全部修复 ✅）

| 问题 | 严重度 | 状态 | 修复方案 |
|---|---|---|---|
| `FactoryScene.ts` 1275 行上帝类 | 🔴 高 | ✅ 已修复 | 拆为 `FactoryRenderer.ts`(519行) + `BuildSystem.ts`(275行) + `BeltEditor.ts`(154行)，FactoryScene 缩至 304 行 |
| 没有任何单元测试 | 🔴 高 | ✅ 已修复 | 添加 vitest，7 个测试文件 100 个测试覆盖 core/ 全层 |
| `Dragon.ts` 动画与渲染混合，464 行 | 🟡 中 | ✅ 已修复 | 拆出 `DragonAnimations.ts`(250行)，Dragon 缩至 162 行 |
| `MachineLogic` 端口数硬编码 3 | 🟡 中 | ✅ 已修复 | 端口数从配方动态推导，`setRecipe()` 自动 resize buffer |
| `src/ui/` 空目录 | 🟡 中 | ✅ 已修复 | 创建 `InventoryHUD`、`BuildMenu`、`RecipePanel` 三个组件 |
| `BeltSegment.maxCapacity=length×2` 硬编码 | 🟢 低 | ✅ 已修复 | 提取为 `GameConfig.BELT_CAPACITY_PER_TILE` |
| `pickIdleBehavior` 无测试 | 🟢 低 | ✅ 已修复 | 参数化测试覆盖 4 种心情的权重分布 |
| 没有全局错误边界 | 🟢 低 | ✅ 已修复 | `window.onerror` + `unhandledrejection` + 友好遮罩 |
| 没有 README | 🟢 低 | ✅ 已修复 | 包含启动说明、技术栈、快捷键表、架构亮点 |
